// components/PathfindingControls.tsx
'use client';
...
import { pathfindingAlgorithms } from '@/lib/utils';
import { runPathfindingAlgorithm } from '@/lib/pathfinding-runner';
...
export default function PathfindingControls() {
  ...
  const canRun = startPoint && endPoint && !isRunning;
  ...
  const handleStart = async () => {
    ...
    try {
      const result = await runPathfindingAlgorithm(
        ...
        startPoint!,
        endPoint!,
        usePathfindingStore.getState().walls,
  ...
  const algorithmInfo = pathfindingAlgorithms[algorithm];
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        ...
        >
          {Object.entries(pathfindingAlgorithms).map(([key, info]) => (
      ...
      {showInfo && (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div>
              <span className="font-medium text-blue-800 dark:text-blue-200">Time:</span>
              <span className="ml-1 text-blue-600 dark:text-blue-400">
                {algorithmInfo.timeComplexity}
              </span>
            </div>
            <div>
              <span className="font-medium text-blue-800 dark:text-blue-200">Space:</span>
              <span className="ml-1 text-blue-600 dark:text-blue-400">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-gray-900 dark:text-white">
            {endPoint ? '✓ Set' : 'Shift + Click on map'}
          </span>
        </div>
      </div>
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3 text-sm">
            ...
            <div className="text-center">
              ...
              <div className="text-green-600 dark:text-green-400 text-xs">Visited</div>
            </div>
            <div className="text-center">
              <div className="font-semibold text-green-800 dark:text-green-200">
                {stats.pathLength}
              </div>
              <div className="text-green-600 dark:text-green-400 text-xs">Path Length</div>
            </div>
          </div>
        </motion.div>
      )}
      ...
      <div className="space-y-3">
        {!isRunning ? (
          ...
          >
            ...
            {!startPoint || !endPoint ? 'Set Start & End Points' : 'Start Pathfinding'}
          </motion.button>
        ) : (
          <motion.button
            whileHover={{ scale: 1.02 }}
